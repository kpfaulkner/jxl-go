...
func NewHybridIntegerConfigWithReader(reader *jxlio.Bitreader, logAlphabetSize int) (*HybridIntegerConfig, error) {
	hic := &HybridIntegerConfig{}
	hic.SplitExponent = int(reader.MustReadBits(uint32(util.CeilLog1p(int64(logAlphabetSize)))))
	if hic.SplitExponent == logAlphabetSize {
		hic.MsbInToken = 0
		hic.LsbInToken = 0
		return hic, nil
	}
	hic.MsbInToken = int(reader.MustReadBits(uint32(util.CeilLog1p(int64(hic.SplitExponent)))))
	if hic.MsbInToken > hic.SplitExponent {
		return nil, errors.New("msbInToken is too large")
	}
	hic.LsbInToken = int(reader.MustReadBits(uint32(util.CeilLog1p(int64(hic.SplitExponent - hic.MsbInToken)))))
