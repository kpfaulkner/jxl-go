...
func (s *XorShiro) fill(bits []int64) {
	for i := 0; i < len(bits); i += 2 {
		l := s.nextLong()
		bits[i] = l & 0xFF_FF_FF_FF
		bits[i+1] = (l >> 32) & 0xFF_FF_FF_FF
	}
}

func (s *XorShiro) nextLong() int64 {
	s.fillBatch()
...
func NewXorShiroWith2Seeds(seed0 int64, seed1 int64) *XorShiro {
	xs := &XorShiro{}
	xs.state0 = make([]int64, 8)
	xs.state1 = make([]int64, 8)
	xs.batch = make([]int64, 8)
	xs.batchPos = 0

	xs.state0[0] = splitMix64(seed0 + 0x9e3779b97f4a7c15)
	xs.state1[0] = splitMix64(seed1 + 0x9e3779b97f4a7c15)
	for i := 1; i < 8; i++ {
		xs.state0[i] = splitMix64(xs.state0[i-1])
		xs.state1[i] = splitMix64(xs.state1[i-1])
	}
	return xs
}

func splitMix64(z int64) int64 {
